...
*** 설명글 : 구제급여 신청 (정보입력) 화면 스크립트
***
*** -----------------------------    Modified Log   --------------------------------------
*** ver             date                author                  description
*** --------------------------------------------------------------------------------------
*** 1.0         2021.10.05    LSH
*** 1.0         2021.11.05    LSH 동의팝업 추가
******************************************************************************************
**/
...
$(function() {
	...
	function doLoadAplyInfo() {
		...
		if (result && 
			...
			// 2023.03.29 이용동의 맵핑
			$('#agreeYn').prop('checked', P_RELIEF['indvInfoClctAgreYn'] == 'Y');
	...
	function doLoadUserInfo() {
		...
		// 2022.12.01 URL 변경
		// 회원정보 가져오기
		//const result = $.ajaxUtil.ajaxDefault(getUrl('/com/cmm/getUserInfo.do'));
		const result = $.ajaxUtil.ajaxDefault(getUrl('/usr/relief/getUserInfo.do'));
    ...
    function doSave( act ) {
		...
		let saveFn = function( saveData, signFile ) {
            ...
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/usr/relief/saveRelief.do'), 
    ...
    function doOpenSurvey() {
		...
		let url = getUrl('/usr/relief/modalSurvey.do')
    ...
    function doOpenSurveyView() {
		...
		C_POPUP_SURVERY= $.commModal.loadView(
			'온라인 설문지 상세보기', 
			getUrl('/usr/relief/modalSurveyView.do?rspnsMngNo='+no), 
			{sizeType: 'large'}
		);
        return false;
    }
	...
	// 이용동의 클릭이벤트 처리
	$('#agreeYn').bind("click", function() {
		$.commModal.loadView('', getUrl('/html/popupReliefAgree.html'), {
			func: function(dialog) {
				
				let body = dialog.getModalBody();
				
				// 2023.03.29 이용동의 체크맵핑
				if ($('#indvInfoClctAgreYn').val() == 'Y') {
					body.find('input.agreeItem[value="Y"]').each(function() {
						$(this).prop('checked', true);
					});
				}
				// 2022.01.18 LSH 전체동의 체크시
