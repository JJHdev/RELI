...
const C_LIST = {
	...
	doInit: function() {
	    ...
	    this.GRID = $('#appGrid').datagrid({
	        ...
	        columns: [[
	            ...
	            {rowspan:2,field:'bizAreaNm'  ,width: 80,halign:'center',align:'center',title:'지역'},
	...
	doSearch: function() {
        ...
        // 그리드 목록조회 URL
        C_LIST.GRID.datagrid('options')['url'] = getUrl('/adm/cmit/getListCmitDmge.do');
        // 검색폼 그리드 검색
	...
	doExcel: function() {
        $.formUtil.submitForm(
            getUrl('/adm/cmit/downCmitDmgeExcel.do'), 
            {formId : "searchForm"}
        );
        return false;
	},
	...
	doLoad: function(index, row) {
		...
		// 상세조회
        $.ajaxUtil.ajaxLoad(
            getUrl('/adm/cmit/getCmitDmge.do'), 
...
const C_CMIT  = {
	...
	doInit: function() {
	    ...
	    this.FORM.validate({
	        ...
	        rules: {
				...
				// 전문위원회 클릭시 피해지역 필수
	            bizAreaCd    : {required: function() { 
									return (c.RADIO.getValue() == CODE.CMIT_SE_CD.EXPERT);
							   }},
		...
		// 피해지역 콤보박스 생성
		this.COMBO = $('#bizAreaCd').appComboBox({
			url: getUrl('/com/cmm/getComboBizMng.do'),
			init: {code:'', text:'해당없음'}
		});
		// 위원회 구분 라디오박스
		this.RADIO = $('#appCmitSeCd').appSelectBox({
			form:    'radio',
			name:    'cmitSeCd', 
			params:  {upCdId: CODE.CMITSE},
			click: function() {
				// 라디오 클릭시 해당항목 표시
				c.doClickCmitSeCd( $(this).val() );
			}
		});
	    // 신규등록버튼 클릭시 이벤트처리
	...
	doLoad: function(data) {
		...
		// 위원회 구분에 따른 설정
		this.doClickCmitSeCd( data['cmitSeCd'] );
	},
	// 신규등록
    //-------------------------------//
	doRegist: function() {
	...
	doRemove: function() {
		...
		$.commMsg.confirm("정말 위원회 정보를 삭제하시겠습니까?", function() {
            ...
            // 등록폼을 AJAX로 저장처리
            f.ajaxForm({
                url: getUrl('/adm/cmit/saveCmitDmge.do'),
                // 오류시 처리로직
	...
	// 위원회구분 클릭시 이벤트 처리
    //-------------------------------//
	doClickCmitSeCd: function( cmitSeCd ) {
		// 전문위원회 클릭시 피해지역 콤보박스 활성화
		if (cmitSeCd == CODE.CMIT_SE_CD.EXPERT)
			C_CMIT.COMBO.enable(); 
		// 그외 피해지역 콤보박스 비활성화
		else
			C_CMIT.COMBO.disable(); 
	},
};
...
const C_MFCMM = {
	...
	doSearch: function() {
		...
		// 목록 로드
       	c.GRID.datagrid('options')['url'] = getUrl('/adm/cmit/getListCmitMng.do');
	...
	doRemove: function() {
		...
		$.commMsg.confirm("정말 삭제하시겠습니까?", function() {
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/adm/cmit/saveCmitMng.do'), 
                JSON.stringify(data),
                function(ret) {
	...
	doRegist: function() {
		...
		P_POPUP = $.commModal.loadView(
			r['cmitSeNm']+' 위원 등록', 
			getUrl('/adm/cmit/modalCmitMng.do')+'?cmitMngNo='+r['cmitMngNo'], 
			{sizeType:'large'}
		);
        return false;
	}
