...
const C_LIST = {
	...
	doInit: function() {
		...
		$('#srchCmitSeCd').combobox({
			...
			queryParams: {upCdId: CODE.BIO_CMITSE},
		...
		$('#srchMfcmmRlmCd').combobox({
			url: getUrl('/com/cmm/getComboCode.do'),
			queryParams: {upCdId: CODE.BIO_MFCMMRLM},
			loadFilter: function(data) {
				data.unshift(COMBO.INIT_ALL);
				return data;
			}
	...
	doSearch: function() {
        ...
        // 그리드 목록조회 URL
        C_LIST.GRID.datagrid('options')['url'] = getUrl('/adm/bio/getListBioMfcmm.do');
        // 검색폼 그리드 검색
	...
	doExcel: function() {
        $.formUtil.submitForm(
            getUrl('/adm/bio/downBioMfcmmExcel.do'), 
            {formId : "searchForm"}
        );
        return false;
	},
	...
	doOpenView: function() {
		...
		$.commModal.loadView(
			'임기 이력 조회 결과', 
			getUrl('/adm/bio/modalBioMfcmmTenure.do')+'?mfcmmNo='+$(this).data('no'), 
			{sizeType:'large'}
		);
        return false;
	},
...
const C_MFCMM  = {
	...
	doInit: function() {
		...
		$('#mfcmmRlmCd').appComboBox({
			url: getUrl('/com/cmm/getComboCode.do'),
			params: {upCdId: CODE.BIO_MFCMMRLM}
		});
		// 연락처 하이픈삽입 이벤트처리
	...
	doSave: function() {
		...
		$.commMsg.confirm("위원정보를 저장하시겠습니까?", function() {
            // 등록폼을 AJAX로 저장처리
            f.ajaxForm({
                url: getUrl('/adm/bio/saveBioMfcmm.do'),
                // 오류시 처리로직
...
const C_COMMON = {
	...
	doSearch: function( TOBJ ) {
		...
		TOBJ.GRID.datagrid('clearSelections');
		// 목록 로드
       	TOBJ.GRID.datagrid('options')['url'] = getUrl('/adm/bio/getListBioMfcmmTenure.do');
	...
	doSave: function( TOBJ ) {
		...
		$.commMsg.confirm(TOBJ.TITLE+" 정보를 저장하시겠습니까?", function() {
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/adm/bio/saveBioMfcmmTenure.do'), 
                JSON.stringify(data),
                function(ret) {
...
const C_JUDGE  = {
	...
	DATA : false,
	// 위원회 구분코드
	CODE : CODE.BIO_CMIT_SE_CD.JUDGE,
	// 탭 명칭
	TITLE : '심사위원회',  
	// KEYS
	KEYS: {
...
const C_REVIEW = {
	...
	DATA : false,
	// 위원회 구분코드
	CODE : CODE.BIO_CMIT_SE_CD.REVIEW,
	// 탭 명칭
	TITLE : '재심사위원회',  
	// KEYS
	KEYS: {
...
$(function() {
	...
	P_TABS   = [C_MFCMM, C_JUDGE, C_REVIEW];
	P_INDEX  = 0;
	P_SELECT = false;
