...
$(function() {
    ...
    P_GRID.datagrid({
        ...
        columns: [[
            ...
            {field:'exmnOder'     ,width:100,halign:'center',align:'center',title:'본조사차수'},
    ...
    function doSearch() {
        ...
        // 그리드 목록조회 URL
        P_GRID.datagrid('options')['url'] = getUrl('/adm/exmn/getListMnsvyPlan.do');
        // 검색폼 그리드 검색
    ...
    function doSelect(index, row) {
        ...
        $.ajaxUtil.ajaxLoad(
            getUrl('/adm/exmn/getMnsvyPlan.do'),
			params,
            function(result) {
    ...
    function doRemove() {
		...
		if (!P_SELECT) {
			$.commMsg.alert('본조사사업 목록에서 항목을 선택하세요.');
			return false;
		}
		if (P_SELECT['exmnCnt'] > 0) {
			$.commMsg.alert('본조사 대상자가 있어 삭제할 수 없습니다.');
			return false;
		}
    	$.commMsg.confirm("정말 삭제하시겠습니까?", function() {
            ...
            // 등록폼을 AJAX로 저장처리
            P_TFORM.ajaxForm({
                url: getUrl('/adm/exmn/saveMnsvyPlan.do'),
                // 오류시 처리로직
    ...
    function doTargetSearch() {
        ...
        // 그리드 목록조회 URL
        P_TGRID.datagrid('options')['url'] = getUrl('/adm/exmn/getListMnsvy.do');
        // 검색폼 그리드 검색
    ...
    function doTargetRegist() {
		...
		popup.openExmn({
			...
			openUrl : getUrl('/adm/exmn/modalMnsvy.do'),
			saveUrl : getUrl('/adm/exmn/saveMnsvy.do'),
