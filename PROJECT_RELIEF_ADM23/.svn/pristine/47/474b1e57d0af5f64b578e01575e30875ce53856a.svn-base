...
$(function() {
    ...
    //--------------------------------------------------------//
    var P_RGRID  = $('#roleGrid'   ); // 역할별 프로그램 그리드 객체
    var P_MGRID  = $('#progGrid'   ); // 프로그램 그리드 객체
    ...
    P_RGRID.datagrid({
        ...
        columns: [[
            ...
            {field:'progNm',width:300,halign:'center',align:'left',title:'프로그램명'},
            {field:'progId',width:180,halign:'center',align:'left',title:'프로그램ID'}
        ]]
    });
    P_MGRID.datagrid({
		// 화면영역 맞춤
    ...
    function doSearchRole() {
        ...
        // 그리드 목록조회 URL
        P_RGRID.datagrid('options')['url'] = getUrl('/sys/role/getListRoleProg.do');
        // 검색폼 그리드 검색
    ...
    // 추가할프로그램목록 검색처리
    //--------------------------------------------------------//
    function doSearchProg() {
		// 선택된 항목 CLEAR
		P_MGRID.datagrid('clearSelections');
        // 검색폼 데이터 객체화
        var obj = P_SFORM.serializeObject();
        // 그리드 목록조회 URL
        P_MGRID.datagrid('options')['url'] = getUrl('/sys/prog/getListProg.do');
        // 검색폼 그리드 검색
    ...
    function doRemove() {
        ...
        if (rows.length == 0) {
            $.easyMsg.alert('제외할 프로그램을 선택하세요.');
            return false;
        }
        
        $.easyMsg.confirm(rows.length+"개의 프로그램을 제외하시겠습니까?", function() {
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/sys/role/saveRoleProg.do'), 
                JSON.stringify({
                    mode: MODE.REMOVE,
                    roleList: rows
                }),
    ...
    function doAppend() {
        ...
        $.easyMsg.confirm(rows.length+"개 프로그램을 추가하시겠습니까?", function() {
            // AJAX로 저장처리
            $.ajaxUtil.ajaxSave(
                getUrl('/sys/role/saveRoleProg.do'), 
                JSON.stringify({
