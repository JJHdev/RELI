...
public class CommController extends BaseController {
    ...
    /**
     * 2022.12.06 LSH
     * 구제급여신청 서류코드 콤보 리스트 조회
     * @param  paramMap.papeCd 상위서류코드 (ex.D00)
     */
    @RequestMapping("/com/cmm/getComboReliefPape.do")
    @ResponseBody
    public List getComboReliefPape(HttpServletRequest request, 
    		ModelMap model) throws Exception {

        Map paramMap = getParameterMap(request, true);
        
        if (!paramMap.containsKey("papeCd"))
        	paramMap.put("papeCd", CommConst.PAPE_RELIEF); //DOO
        
        // -------------------- Default Setting End -----------------------//
        return commService.getListPapeCode(paramMap);
    }
     ...
     * 공휴일 연도 Combo목록 조회
     */
    @RequestMapping("/com/cmm/getComboHldyYear.do")
    @ResponseBody
    public List getComboHldyYear(HttpServletRequest request
    		, @ModelAttribute ModelMap model) throws Exception {

        Map paramMap = getParameterMap(request, true);
        // -------------------- Default Setting End -----------------------//
        return commService.getComboHldyYear(paramMap);
    }
