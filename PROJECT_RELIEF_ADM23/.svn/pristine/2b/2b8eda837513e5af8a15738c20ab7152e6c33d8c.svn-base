/**
*******************************************************************************
*** 파일명 : modalMfcmmTenure.js
*** 설명글 : 위원관리 - 임기이력조회 팝업 화면 스크립트
***
*** -----------------------------    Modified Log   ---------------------------
...
$(function() {
	...
	let P_GRID = $('#p_tenureGrid').appTableLayout({
		...
		titleTag:  'h3',
		tailSpace: true,
		colgroup:  ['25%','10%','25%','30%'],
		columns: [
			{name: 'cmitSeNm'     , label: '위원회 구분' },
			{name: 'bizAreaNm'    , label: '지  역'      },
	...
	let data = $.ajaxUtil.ajaxDefault(getUrl('/adm/cmit/getMfcmm.do'), params);
	if (data && data['Data']) {
		P_INFO.loadData([data['Data']]);
	    $.ajaxUtil.ajaxLoad(
	        getUrl('/adm/cmit/getListMfcmmTenure.do'), 
			params,
	        function(result) {
